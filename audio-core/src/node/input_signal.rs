...
use dasp::{
    graph::{BoxedNodeSend, Buffer, Input, Node, NodeData},
    Frame, Signal,
};
use std::sync::Arc;
    ...
    S: Signal<Frame = f32> + Send,
{
    ...
    {
        let storage: [Arc<AtomicFloat>; N] = [(); N].map(|()| Arc::new(AtomicFloat::new(0.0)));

        let mut signal: [InputSignal; N] = storage.clone().map(InputSignal);
    ...
    S: Signal<Frame = f32> + Send,
{
    fn process(&mut self, inputs: &[Input], output: &mut [Buffer]) {
        ...
        for ix in 0..Buffer::LEN {
            ...
            let input_frames = input_buffers.iter().map(|buffer| &buffer[0][ix]).collect();
...
impl<S, const N: usize> From<InputSignalNode<S, N>> for NodeData<AudioNode>
where
    S: Signal<Frame = f32> + Send + 'static,
{
    fn from(node: InputSignalNode<S, N>) -> Self {
        NodeData::new1(AudioNode::Signal(BoxedNodeSend::new(node)))
    }
}

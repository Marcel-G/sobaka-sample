...
use crate::{graph::{AudioGraph, NodeIndex}, node::{input_signal::InputSignalNode}};
...
impl Module for EnvelopeModule {
    ...
    fn create(&mut self, core: &mut AudioGraph) {
      ...
      let envelope = core.add_node(
        InputSignalNode::new(|[gate, a, d, s, r]| {
      ...
      for (name, input) in [gate, attack, decay, sustain, release] {
        core.add_edge(
          input.node.expect("Input not initialised"),
          envelope
        );

...
use crate::{
    ...
    node::AudioNode,
    util::state_observer::{ObserveState, Observer, ObserverStorage},
};
...
impl OscillatorModule {
    fn create_sin(&mut self, core: &mut AudioGraph) -> NodeIndex {
        core.add_node(InputSignalNode::new(|[frequency]| {
...
impl StatefulModule for OscillatorModule {
    ...
    fn update(&mut self, core: &mut AudioGraph, state: Self::State) {
        if self.wave != state.wave {
            ...
            core.add_edge(
                self.input(&OscillatorInput::Frequency)
                    .expect("Input not initialised")
                    .node
                    .unwrap(),
                new_oscillator,
            );
            ...
            core.add_edge(
                ...
                self.output
                    .as_ref()
                    .expect("Output is not initialised")
                    .node,
            );

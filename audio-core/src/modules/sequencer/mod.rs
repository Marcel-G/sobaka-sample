...
use crate::{graph::{AudioGraph, NodeIndex}, node::AudioNode, util::state_observer::{ObserveState, Observer}};
...
impl StatefulModule for SequencerModule {
  type State = SequencerState;
	fn create(&mut self, core: &mut AudioGraph, initial_state: Self::State) {
		...
		core.add_edge(
			gate.node.expect("Input not initialised"),
			node
		);
	...
	fn subscribe(&self, core: &AudioGraph) -> Option<Observer<Self::State>> {
		if let Some(sequencer_node) = self.node(core) {
			Some(sequencer_node.observe())
		} else {
			None
		}
	}

...
use crate::{
    node::AudioNode,
    util::state_observer::{ObserveState, ObserverStorage},
};
...
impl From<SequencerNode> for NodeData<AudioNode> {
    fn from(node: SequencerNode) -> Self {
        NodeData::new1(AudioNode::Sequencer(node))
    }
}

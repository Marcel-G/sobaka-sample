<script lang="ts">
	import { Envelope, EnvelopeInput, Parameter, ParameterState, SamplerNode } from "sobaka-sample-web-audio";
  ...
  export let context: SamplerNode;
	...
	const loading = Promise.all([
		...
		.then(([envelope_id, attack_id, decay_id, sustain_id, release_id]) => {
			modules.register(id, {
...
>
	...
	{:then} 
		<Knob
			label="Attack"
			bind:value={$attack.value}
			bind:range={$attack.range}
		/>
		<Knob
			label="Decay"
			bind:value={$decay.value}
			bind:range={$decay.range}
		/>
		<Knob
			label="Sustain"
			bind:value={$sustain.value}
			bind:range={$sustain.range}
		/>
		<Knob
			label="Release"
			bind:value={$release.value}
			bind:range={$release.range}
		/>
	{/await}

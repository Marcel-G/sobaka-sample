...
export abstract class AbstractModule<T extends ModuleType> {
  ...
  constructor(context: SamplerNode, type: T) {
	...
	async dispose(): Promise<boolean> {
		let module_id = await this.get_module_id(); 
...
export abstract class AbstractStatefulModule<T extends ModuleType, State> extends AbstractModule<T> {
	...
	async subscribe(callback: Subscriber<State>): Promise<Unsubscriber> {
		...
		let unsubscribe: Unsubscriber | null = this.get_context()
			.subscribe(
				'module/subscribe',
				'module/unsubscribe',
				[module_id],
	...
	async dispose(): Promise<boolean> {
		this.unsubscribe_handles.forEach((unsubscribe) => { unsubscribe() });

...
use sobaka_sample_audio_core::{graph::EdgeIndex, modules::{AudioModule, clock::ClockModule, envelope::EnvelopeModule, oscillator::OscillatorModule, parameter::ParameterModule, sequencer::SequencerModule, sink::SinkModule, traits::{Module, StatefulModule}, volume::VolumeModule}};
...
impl ModuleRpc for RpcImpl {
        ...
        if let Some(state) = initial_state {
            match &mut module {
                AudioModule::Oscillator(oscillator) => {
                    StatefulModule::create(
                        oscillator,
                        &mut core.graph,
                        state.try_into().map_err(into_err)?
                    )
                }
                AudioModule::Parameter(parameter) => {
                    StatefulModule::create(
                        parameter,
                        &mut core.graph,
                        state.try_into().map_err(into_err)?
                ...
                AudioModule::Sequencer(sequencer) => {
                    StatefulModule::create(
                        sequencer,
                        &mut core.graph,
                        state.try_into().map_err(into_err)?
        ...
        if let Some(module) = core.modules.get(module_id) {
            let result = match module {
                AudioModule::Oscillator(oscillator) => Ok(oscillator
                    ...
                    .map(|state| ModuleStateDTO::Oscillator(state))
                    .boxed()
                ),
                AudioModule::Parameter(parameter) => Ok(parameter
                    .subscribe(&core.graph)
                    .expect("Could not subscribe") // @todo this should not be optional
                    .map(|state| ModuleStateDTO::Parameter(state))
                    .boxed()
                ),
                AudioModule::Sequencer(sequencer) => Ok(sequencer
                    .subscribe(&core.graph)
                    .expect("Could not subscribe") // @todo this should not be optional
                    .map(|state| ModuleStateDTO::Sequencer(state))
                    .boxed()
                ),
    ...
    fn connect_module(&self, module_id_a: usize, module_id_b: usize, input_name: InputTypeDTO ) -> Result<usize>  {
        let (output, input) = {
            let core = self.core.lock().expect("Cannot lock core");
            let module_a = core.modules.get(module_id_a).ok_or("Module a not found").map_err(into_err)?;
            let module_b = core.modules.get(module_id_b).ok_or("Module b not found").map_err(into_err)?;

            let output = module_a.output().expect("No output found");

            let input = match module_b {
                AudioModule::Clock(module) => { module.input(&input_name.try_into().map_err(into_err)?) },
                AudioModule::Envelope(module) => { module.input(&input_name.try_into().map_err(into_err)?) },
                AudioModule::Oscillator(module) => { module.input(&input_name.try_into().map_err(into_err)?) },
                AudioModule::Sequencer(module) => { module.input(&input_name.try_into().map_err(into_err)?) },
                AudioModule::Sink(module) => { module.input(&input_name.try_into().map_err(into_err)?) },
                AudioModule::Volume(module) => { module.input(&input_name.try_into().map_err(into_err)?) },
                _ => { None }
            };

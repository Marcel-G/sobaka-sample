...
mod filters {
    ...
    #[test]
    fn test_pad() {
        assert_eq!(pad(&[1, 2, 3, 4, 5], 1), vec![1, 1, 2, 3, 4, 5]);

        assert_eq!(pad(&[1, 2, 3, 4, 5], 2), vec![1, 1, 2, 3, 4, 5, 5]);

        assert_eq!(pad(&[1, 2, 3, 4, 5], 3), vec![1, 1, 1, 2, 3, 4, 5, 5]);
    }
    ...
    fn test_uniform_filter() {
        ...
        //
        assert_eq!(
            uniform_filter(&[2.0, 8.0, 0.0, 4.0, 1.0, 9.0, 9.0, 0.0], 3),
            vec![4.0, 3.333_333_3, 4.0, 1.666_666_6, 4.666_666_5, 6.333_333_5, 6.0, 3.0]
        );
        ...
        // ```
        assert_eq!(
            uniform_filter(&[2.0, 8.0, 0.0, 4.0, 1.0, 9.0, 9.0, 0.0], 2),
    ...
    fn test_maximum_filter() {
        ...
        // ```
        assert_eq!(
            maximum_filter(&[31., 41., 59., 26., 53., 58., 97.], 3),
...
#[cfg(test)]
mod odf_tests {
    use crate::dsp::onset::{hanning, onset, superflux_diff_spec};

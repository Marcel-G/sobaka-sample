...
impl WorkerPool {
    ...
    pub fn new(
        ...
        let pool = WorkerPool {
            worker_url: bindgen_entry.to_string(),
            worker_options,
            state: Rc::new(PoolState {
                workers: RefCell::new(Vec::with_capacity(initial)),
                callback: Closure::new(|event: Event| {
                    console::log_1(&format!("unhandled event: {}", event.type_()).into());
                }),
            }),
        };
        for _ in 0..initial {
    ...
    fn spawn(&self) -> Result<Worker, JsValue> {
        ...
        let worker = Worker::new_with_options(&self.worker_url, &self.worker_options)?;
    ...
    fn reclaim_on_message(&self, worker: Worker) {
        ...
        let reclaim = Closure::<dyn FnMut(_)>::new(move |event: Event| {
            if let Some(error) = event.dyn_ref::<ErrorEvent>() {
                console::log_1(&format!("error in worker: {}", error.message()).into());

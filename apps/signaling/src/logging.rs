//! Logging configuration for the signaling server.
//!
...
//!
//! - `RUST_LOG`: Controls log level filtering (default: `info,signaling=debug`)
//!   Examples:
//!   - `RUST_LOG=debug` - Enable debug for all crates
//!   - `RUST_LOG=warn,signaling=info` - Warn for deps, info for signaling
//!   - `RUST_LOG=signaling::signaling=trace` - Trace only for signaling module
//!
...
pub fn init() {
    let env_filter = EnvFilter::try_from_default_env()
        .unwrap_or_else(|_| EnvFilter::new("info,signaling=debug"));
    ...
    tracing::info!(
        ...
        "Signaling server logging initialized"
    );
}

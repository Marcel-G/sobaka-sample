...
use jsonrpc_pubsub::{typed::Subscriber, PubSubHandler, Session};
use module::{AudioModuleType};
use petgraph::graph::EdgeIndex;
use std::{sync::{Arc, Mutex, atomic::AtomicUsize}, collections::HashMap};
use utils::subscriptions::Subscriptions;
...
impl AudioProcessor {
    ...
    pub fn new(port: MessagePort, sample_rate: f64) -> Self {
        ...
        processor.init_rpc(port);

        processor
    }

    fn init_rpc(&self, port: MessagePort) {
        ...
        let metadata_extractor =
            |sender: &UnboundedSender<String>| {
                Arc::new(Session::new(sender.clone()))
            };
